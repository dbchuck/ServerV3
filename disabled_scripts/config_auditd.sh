...
# Make sure the auditd service is always running
systemctl enable --now auditd.service

# Start the audit program before
sed -i "s@.*GRUB_CMDLINE_LINUX.*@$(echo GRUB_CMDLINE_LINUX=\"$(cat /etc/default/grub | grep GRUB_CMDLINE_LINUX | awk -F\" '{print $2,"audit=1"}')\")@" /etc/default/grub

set_value num_logs 10
set_value max_log_file 50MB
set_value max_log_file_action rotate
set_value space_left_action email
set_value admin_space_left_action halt
set_value action_mail_acct $AUDIT_EMAIL
